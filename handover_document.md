...
├── _connect_flow_snapshots/         ← timestamped JSON snapshots of GI_Inbound_Main
│   ├── *.json                       ← pre-change content + pretty-printed copies
│   └── ROLLBACK_GoAhead.ps1         ← one-paste rollback for the "Go ahead" flow change
└── *.zip                            ← deployment artifacts (gitignored in spirit)
```
...
Every change to `GI_Inbound_Main` is preceded by a timestamped JSON
snapshot in `_connect_flow_snapshots/`. The 2026-05-24 "Go ahead"
prompt change has a one-paste restore:
...
| --- | --- | --- | --- |
| Caller dials, hears nothing after greeting | Connect flow stuck (block failure, missing attribute, Lex alias misconfigured) | Connect → **Contact Search** → open the contact → **Contact flow events** | Identify the block that errored; check its referenced attributes/intents exist. Check `_connect_flow_snapshots/` for the last-known-good content. |
...
| Change the IVR (greeting, language gate, collection prompts) | AWS Connect console → flows → `GI_Inbound_Main`. Snapshot first; rollback artifacts go to `_connect_flow_snapshots/` |
